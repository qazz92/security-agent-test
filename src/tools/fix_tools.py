...
@tool
def generate_fix_code(vulnerability: Dict[str, Any]) -> Dict[str, str]:
    """
...
@tool
def create_pr_template(vulnerabilities: List[Dict[str, Any]], project_info: Dict[str, Any] = None) -> str:
    """
    GitHub Pull Request 템플릿을 생성합니다.

    Args:
        vulnerabilities: 수정된 취약점 목록
...
@tool
def generate_security_documentation(vulnerabilities: List[Dict[str, Any]], fixes: List[Dict[str, Any]]) -> Dict[str, str]:
    """
    보안 수정에 대한 문서를 생성합니다.
...
"""
        ...
        # 4. 보안 테스트 가이드
        security_testing_guide = """# Security Testing Guide
...
### Pre-commit Hooks
Add these security checks to your pre-commit configuration:
...
Test these payloads against your endpoints:
- `'; DROP TABLE users; --`
- `1' OR '1'='1`
...
All security fixes should include corresponding test cases to prevent regression.
"""

        docs['SECURITY_TESTING.md'] = security_testing_guide
...
@tool
def generate_fix_script(vulnerabilities: List[Dict[str, Any]]) -> str:
    """
    ...
    try:
        ...
        # 종속성 업데이트
        has_dependency_vulns = any('requirements.txt' in v.get('file', '') for v in vulnerabilities)
        if has_dependency_vulns:
            script_lines.extend([
                ...
                "    sed -i 's/Flask==1.0.0/Flask>=2.3.0/' requirements.txt",
                "    sed -i 's/requests==2.19.0/requests>=2.31.0/' requirements.txt",
                "    sed -i 's/PyYAML==3.13/PyYAML>=6.0.1/' requirements.txt",
                "    sed -i 's/Jinja2==2.10/Jinja2>=3.1.0/' requirements.txt",
                "    ",
        ...
        # Docker 관련 수정
        has_docker_issues = any('Dockerfile' in v.get('file', '') for v in vulnerabilities)
        if has_docker_issues:
            script_lines.extend([
                "# Fix Docker security issues",
                "if [ -f Dockerfile ]; then",
                "    echo 'Fixing Dockerfile security issues...'",
                "    # Backup original Dockerfile",
                "    cp Dockerfile Dockerfile.bak",
                "    ",
                "    # Update base image",
                "    sed -i 's/python:3.6-slim/python:3.11-slim/' Dockerfile",
                "    ",
                "    # Add non-root user",
                "    if ! grep -q 'adduser' Dockerfile; then",
                "        echo 'Adding non-root user to Dockerfile...'",
                "        sed -i '/WORKDIR/i\\RUN adduser --disabled-password --gecos \"\" appuser' Dockerfile",
